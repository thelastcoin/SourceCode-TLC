...
uint256 hashGenesisBlock("0x000009ce5a6591e5d49a0b2210ac990ee28decd087684f8d7c7f588ab6d9c908");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TheLastCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TheLastCoin Signed Message:\n";
...
{
    // TheLastCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    else if(nHeight<=2250000)
    {
        nSubsidy = 4 *COIN;
    }
    else if(nHeight<=2950000)
    {
        nSubsidy = 2 *COIN;
    }
    else if(nHeight>3650000)
    {
        nSubsidy = 1 *COIN;
    }
...
static const int64 nTargetTimespan = 1 * 90; // 90 second blocks (1.5 minutes) Retargget
static const int64 nTargetSpacing = 1 * 90; // 90 Second blocks (1.5 minutes)
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TheLastCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, thelastcoin - DarkGravity v2, written by Evan Duffield - evan@thelastcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, thelastcoin - DarkGravity v3, written by Evan Duffield - evan@thelastcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // TheLastCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // TheLastCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "The last coin is here";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x31aa69d7ca0cfa9da948af2ea9dbe3f307ae03ad6645f2e673b7e6781523d492"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TheLastCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TheLastCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TheLastCoinMiner(CWallet *pwallet)
{
    printf("TheLastCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("thelastcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TheLastCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TheLastCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TheLastCoinMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - THELASTCOIN **********
    Copyright 2014, Thelastcoin Developers 
        eduffield - evan@thelastcoin.io
        InternetApe - kyle@thelastcoin.io
*/
